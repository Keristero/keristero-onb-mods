...
--Version 2.3 (added update_offset, made offsets optional in spawn_visual_artifact)
...
function battle_helpers.spawn_visual_artifact(character,tile,texture,animation_path,animation_state,position_x,position_y,dont_flip_offset)
    ...
    if position_x or position_y then
        battle_helpers.update_offset(visual_artifact,position_x,position_y,dont_flip_offset)
    end
    anim:refresh(sprite)
...
function battle_helpers.update_offset(entity,position_x,position_y,dont_flip_offset)
    local facing = entity:get_facing()
    if facing == Direction.Left and not dont_flip_offset then
        position_x = position_x *-1
    end
    entity:set_offset(position_x,position_y)
end
...
function battle_helpers.drop_trace_fx(target_artifact,lifetime_ms,x_vel,y_vel)
    ...
    fx:set_offset(x_offset,y_offset)
    ...
    fx.update_func = function(self, dt)
        ...
        self:set_color(Color.new(c.r, c.g, c.b,alpha))
        if x_vel ~= 0 then
            x_offset = x_offset+x_vel
        end
        if y_vel ~= 0 then
